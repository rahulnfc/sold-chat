...
const PostDetail = () => {
  ...
  const fetchPost = useCallback(async () => {
    try {
      const response = await axios.get(`https://sold.dxg.world/api/ad/${id}`);
  ...
  const handleStartChat = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://sold.dxg.world/api/conversations', { postId: post._id, postOwnerId: post.postedBy._id }, { headers });
